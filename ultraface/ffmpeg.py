...
from ultraface import logger, process_manager
from ultraface.typing import OutputVideoPreset, Fps, AudioBuffer
from ultraface.filesystem import get_temp_frames_pattern, get_temp_output_video_path
from ultraface.vision import restrict_video_fps
...
def run_ffmpeg(args : List[str]) -> bool:
	...
	while process_manager.is_processing():
		try:
			if ultraface.globals.log_level == 'debug':
				log_debug(process)
...
def extract_frames(target_path : str, temp_video_resolution : str, temp_video_fps : Fps) -> bool:
	trim_frame_start = ultraface.globals.trim_frame_start
	trim_frame_end = ultraface.globals.trim_frame_end
...
def merge_video(target_path : str, output_video_resolution : str, output_video_fps : Fps) -> bool:
	...
	temp_frames_pattern = get_temp_frames_pattern(target_path, '%04d')
	commands = [ '-r', str(temp_video_fps), '-i', temp_frames_pattern, '-s', str(output_video_resolution), '-c:v', ultraface.globals.output_video_encoder]

	if ultraface.globals.output_video_encoder in ['libx264', 'libx265']:
		output_video_compression = round(51 - (ultraface.globals.output_video_quality * 0.51))
		commands.extend([ '-crf', str(output_video_compression), '-preset', ultraface.globals.output_video_preset])
	if ultraface.globals.output_video_encoder in ['libvpx-vp9']:
		output_video_compression = round(63 - (ultraface.globals.output_video_quality * 0.63))
		commands.extend([ '-crf', str(output_video_compression) ])
	if ultraface.globals.output_video_encoder in ['h264_nvenc', 'hevc_nvenc']:
		output_video_compression = round(51 - (ultraface.globals.output_video_quality * 0.51))
		commands.extend([ '-cq', str(output_video_compression), '-preset', map_nvenc_preset(ultraface.globals.output_video_preset)])
	if ultraface.globals.output_video_encoder in ['h264_amf', 'hevc_amf']:
		output_video_compression = round(51 - (ultraface.globals.output_video_quality * 0.51))
		commands.extend([ '-qp_i', str(output_video_compression), '-qp_p', str(output_video_compression), '-quality', map_amf_preset(ultraface.globals.output_video_preset)])
...
def finalize_image(output_path : str, output_image_resolution : str) -> bool:
	output_image_compression = round(31 - (ultraface.globals.output_image_quality * 0.31))
