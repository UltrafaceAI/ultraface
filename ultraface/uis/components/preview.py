...
from ultraface import logger, wording
from ultraface.audio import get_audio_frame, create_empty_audio_frame
from ultraface.common_helper import get_first
from ultraface.core import conditional_append_reference_faces
from ultraface.face_analyser import get_average_face, clear_face_analyser
from ultraface.face_store import clear_static_faces, get_reference_faces, clear_reference_faces
from ultraface.typing import Face, FaceSet, AudioFrame, VisionFrame
from ultraface.vision import get_video_frame, count_video_frame_total, normalize_frame_color, resize_frame_resolution, read_static_image, read_static_images
from ultraface.filesystem import is_image, is_video, filter_audio_paths
from ultraface.content_analyser import analyse_frame
from ultraface.processors.frame.core import load_frame_processor_module
from ultraface.uis.core import get_ui_component, get_ui_components, register_ui_component
...
def render() -> None:
	...
	if source_audio_path and ultraface.globals.output_video_fps and ultraface.globals.reference_frame_number:
		temp_audio_frame = get_audio_frame(source_audio_path, ultraface.globals.output_video_fps, ultraface.globals.reference_frame_number)
		if numpy.any(temp_audio_frame):
			source_audio_frame = temp_audio_frame
	if is_image(ultraface.globals.target_path):
		target_vision_frame = read_static_image(ultraface.globals.target_path)
	...
	if is_video(ultraface.globals.target_path):
		temp_vision_frame = get_video_frame(ultraface.globals.target_path, ultraface.globals.reference_frame_number)
		preview_vision_frame = process_preview_frame(reference_faces, source_face, source_audio_frame, temp_vision_frame)
		preview_image_args['value'] = normalize_frame_color(preview_vision_frame)
		preview_image_args['visible'] = True
		preview_frame_slider_args['value'] = ultraface.globals.reference_frame_number
		preview_frame_slider_args['maximum'] = count_video_frame_total(ultraface.globals.target_path)
...
def update_preview_image(frame_number : int = 0) -> gradio.Image:
	for frame_processor in ultraface.globals.frame_processors:
	...
	source_audio_frame = create_empty_audio_frame()
	if source_audio_path and ultraface.globals.output_video_fps and ultraface.globals.reference_frame_number:
		reference_audio_frame_number = ultraface.globals.reference_frame_number
		if ultraface.globals.trim_frame_start:
			reference_audio_frame_number -= ultraface.globals.trim_frame_start
		temp_audio_frame = get_audio_frame(source_audio_path, ultraface.globals.output_video_fps, reference_audio_frame_number)
		if numpy.any(temp_audio_frame):
			source_audio_frame = temp_audio_frame
	if is_image(ultraface.globals.target_path):
		target_vision_frame = read_static_image(ultraface.globals.target_path)
	...
	if is_video(ultraface.globals.target_path):
		temp_vision_frame = get_video_frame(ultraface.globals.target_path, frame_number)
		preview_vision_frame = process_preview_frame(reference_faces, source_face, source_audio_frame, temp_vision_frame)
		preview_vision_frame = normalize_frame_color(preview_vision_frame)
		return gradio.Image(value = preview_vision_frame)
