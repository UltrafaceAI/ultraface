...
from ultraface.vision import get_video_frame, read_static_image, normalize_frame_color
from ultraface.filesystem import is_image, is_video
from ultraface.face_analyser import get_many_faces
from ultraface.typing import VisionFrame, FaceSelectorMode
...
def render() -> None:
	...
	reference_face_gallery_args : Dict[str, Any] =\
	{
		...
		'columns': 8,
		'allow_preview': False,
		'visible': 'reference' in ultraface.globals.face_selector_mode
	}
	if is_image(ultraface.globals.target_path):
		reference_frame = read_static_image(ultraface.globals.target_path)
	...
	FACE_SELECTOR_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = ultraface.choices.face_selector_modes,
		value = ultraface.globals.face_selector_mode
	)
	REFERENCE_FACE_POSITION_GALLERY = gradio.Gallery(**reference_face_gallery_args)
	REFERENCE_FACE_DISTANCE_SLIDER = gradio.Slider(
		label = wording.get('uis.reference_face_distance_slider'),
		value = ultraface.globals.reference_face_distance,
		step =ultraface.choices.reference_face_distance_range[1] - ultraface.choices.reference_face_distance_range[0],
		minimum = ultraface.choices.reference_face_distance_range[0],
		maximum = ultraface.choices.reference_face_distance_range[-1],
		visible = 'reference' in ultraface.globals.face_selector_mode
	)
...
def update_face_selector_mode(face_selector_mode : FaceSelectorMode) -> Tuple[gradio.Gallery, gradio.Slider]:
	if face_selector_mode == 'many':
		ultraface.globals.face_selector_mode = face_selector_mode
...
def update_reference_face_position(reference_face_position : int = 0) -> None:
	ultraface.globals.reference_face_position = reference_face_position


def update_reference_face_distance(reference_face_distance : float) -> None:
	ultraface.globals.reference_face_distance = reference_face_distance


def update_reference_frame_number(reference_frame_number : int) -> None:
	ultraface.globals.reference_frame_number = reference_frame_number
...
def update_reference_position_gallery() -> gradio.Gallery:
	...
	if is_image(ultraface.globals.target_path):
		temp_vision_frame = read_static_image(ultraface.globals.target_path)
