...
from ultraface.audio import create_empty_audio_frame
from ultraface.content_analyser import analyse_stream
from ultraface.filesystem import filter_image_paths
from ultraface.typing import VisionFrame, Face, Fps
from ultraface.face_analyser import get_average_face
from ultraface.processors.frame.core import get_frame_processors_modules, load_frame_processor_module
from ultraface.ffmpeg import open_ffmpeg
from ultraface.vision import normalize_frame_color, read_static_images, unpack_resolution
from ultraface.uis.typing import StreamMode, WebcamMode
from ultraface.uis.core import get_ui_component, get_ui_components
...
def start(webcam_mode : WebcamMode, webcam_resolution : str, webcam_fps : Fps) -> Generator[VisionFrame, None, None]:
	ultraface.globals.face_selector_mode = 'one'
	ultraface.globals.face_analyser_order = 'large-small'
...
def multi_process_capture(source_face : Face, webcam_capture : cv2.VideoCapture, webcam_fps : Fps) -> Generator[VisionFrame, None, None]:
	with tqdm(desc = wording.get('processing'), unit = 'frame', ascii = ' =', disable =ultraface.globals.log_level in ['warn', 'error']) as progress:
		with ThreadPoolExecutor(max_workers = ultraface.globals.execution_thread_count) as executor:
			futures = []
